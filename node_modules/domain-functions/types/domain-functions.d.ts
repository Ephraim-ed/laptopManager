import type { DomainFunction, ErrorData, Last, MergeObjs, Result, TupleToUnion, UnpackAll, UnpackData, UnpackDFObject, UnpackResult } from './types.js';
/**
 ...
 * }
 */
declare function safeResult<T>(fn: () => T): Promise<Result<T>>;
/**
 ...
 */
declare function applyEnvironment<Fn extends (input: unknown, environment: unknown) => unknown>(df: Fn, environment: unknown): (input: unknown) => ReturnType<Fn>;
/**
 ...
 */
declare function collect<Fns extends Record<string, DomainFunction>>(fns: Fns): DomainFunction<UnpackDFObject<Fns>>;
/**
 ...
 * //    ^? DomainFunction<{ a: string, b: number }>
 */
declare function merge<Fns extends DomainFunction<Record<string, unknown>>[]>(...fns: Fns): DomainFunction<MergeObjs<UnpackAll<Fns>>>;
/**
 ...
 */
declare function pipe<T extends DomainFunction[]>(...fns: T): DomainFunction<Last<UnpackAll<T>>>;
/**
...
//    ^? DomainFunction<{ a: string, b: number }>
 */
declare function collectSequence<Fns extends Record<string, DomainFunction>>(fns: Fns): DomainFunction<UnpackDFObject<Fns>>;
/**
 ...
 */
declare function mapError<O>(dfn: DomainFunction<O>, mapper: (element: ErrorData) => ErrorData | Promise<ErrorData>): DomainFunction<O>;
type TraceData<T> = {
    input: unknown;
    environment: unknown;
    result: T;
...
export { all, applyEnvironment, branch, collect, collectSequence, first, fromSuccess, map, mapError, merge, pipe, safeResult, sequence, trace, };
