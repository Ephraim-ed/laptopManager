...
exports.inputFromUrl = exports.inputFromSearch = exports.inputFromFormData = exports.inputFromForm = void 0;
/**
 ...
 * //    ^? { a: '1', b: '2' }
 */
const inputFromSearch = (queryString) => {
    const pairs = [];
    ...
    return pairs
        ...
        if (compositeKey) {
            ...
            const placeValue = (current, keys, value) => {
                if (keys.length > 1) {
                    // we still have at least 1 nested key
                    const [nextKey, ...rest] = keys;
                    const initialValueFromKeys = (nextKeys) => typeof nextKeys[0] === 'string' && !isNaN(Number(nextKeys[0]))
                        ? []
                        : {};
                    if (current instanceof Array) {
                        ...
                        placeValue(current[arrayKey], rest, value);
                    }
                    else {
                        if (!current[nextKey]) {
                            current[nextKey] = initialValueFromKeys(rest);
                        }
                        placeValue(current[nextKey], rest, value);
                    }
                }
                else {
 ...
 * //    ^? { a: '1', b: '2' }
 */
const inputFromFormData = (formData) => inputFromSearch(new URLSearchParams(formData));
